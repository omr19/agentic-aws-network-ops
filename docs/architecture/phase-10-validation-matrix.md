...
|---|---|---|---|
| Terraform Phase 8 readiness IAM policy uses account `*` in EC2 resource ARNs, while `iam/phase8/remediation-write-permissions.json` and ADR 022 require exact account scope. | **Open** | IAM/security owner | Reconcile the Terraform policy with exact account scope and add rendered-policy coverage under an authorized IAM/Terraform task. No policy was changed here. |
